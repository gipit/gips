...
namespace gip {
	...
	*/
	...
	public:
		...
		template<class T> cimg_library::CImg<unsigned char> SaturationMask(int chunk=0) const {
		    cimg_library::CImg<float> band(ReadRaw<T>(chunk));
		    return band.threshold(_maxDC);
		}

		//! NoData mask: 1's where it's good data
		template<class T> cimg_library::CImg<unsigned char> NoDataMask(int chunk=0) const {
		    using cimg_library::CImg;
		    CImg<T> img = ReadRaw<T>(chunk);
