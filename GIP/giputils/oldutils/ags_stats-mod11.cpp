...
int main (int ac, char* av[]) {
	...
	for (int i=0;i<Opts.InputFiles().size(); i++) {
		...
		Image[0].SetGainOffset(0.02);
		Image[2].SetGainOffset(0.1);
		//string year= Image.Basename().substr(13,4);
		//string doy = Image.Basename().substr(33,3);

		// Pointer to cloud band
		//cout << "cloudband = " << cloudband << endl;
		//cloud = NULL;
		//if (cloudband > 0) cloud =  &(Image[cloudband-1]);
		...
		for (int b=0;b<Opts.Bands().size(); b++) {
			...
			//if (Opts.NoData()) {
				...
				Masks.push_back( GeoMask<T>(&Image[Opts.Bands(b)-1],Opts.NoDataValue(),false) );
