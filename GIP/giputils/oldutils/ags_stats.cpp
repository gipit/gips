...
int main (int ac, char* av[]) {
	...
	int wsm(8);
	int w(12);
	cout << setw(wsm) << "FID" << setw(wsm) << "NumPix"; //<< setw(wsm) << "Year" << setw(wsm) << "DOY";
	...
	for (int i=0;i<Opts.InputFiles().size(); i++) {
		...
		for (int id=id_lo;id<=id_hi;id++) {
			...
			int numpixels = 0;
			for (int b=0;b<Opts.Bands().size(); b++) {
				map<T,POI>::iterator iPOI = POIs[Opts.Bands(b)-1].find(id);
			...
			if (numpixels > 0) {
				// ID, Year, doy
				cout << setw(wsm) << id << setw(wsm) << Stats[0].NumPixels(); // << setw(wsm) << year << setw(wsm) << doy;
				for (int b=0;b<Opts.Bands().size();b++) {
