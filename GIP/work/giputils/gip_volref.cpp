...
int main (int ac, char* av[]) {
	...
	localopts.add_options()
		...
		("p", bpo::value<float>()->default_value(0.03), "Internal Fresnel reflectance")
		("pp", bpo::value<float>()->default_value(0.54), "Water-air Fresnel reflectance")
		("n", bpo::value<float>()->default_value(1.34), "Refractive Index of water")
		("Q", bpo::value<float>()->default_value(1.0), "Downwelled Irradiance/Upwelled Radiance")
	...
	if (Opts.Verbose()) {
		cout << "Water column Reflectance" << endl;
        cout << "\tInput File: " << Opts.InputFile(0) << endl;
		cout << "\tOutput File: " << Opts.OutputFile(1) << endl;
        cout << "\tSpecular reflectance band: " << srband << endl;
	}
