...
class CDLData(Data):
    ...
    _legend_file = os.path.join(_rootdir + 'CDL_Legend.csv')
    ...
    def find_assets(self, tile):
        return glob.glob(os.path.join(self._tiledir, tile, 'CDL_%s_*.tif' % self.date.strftime('%Y')))
    ...
    def find_dates(cls, tile):
        ...
        files = glob.glob(os.path.join(cls._tiledir,tile,'CDL*.tif'))
