...
class LandsatInventory(DataInventory):
    _colors = {'Landsat4':'bright yellow', 'Landsat5':'bright red', 'Landsat7':'bright green', 'Landsat8':'bright blue'}
    ...
    def __init__(self, site=None, tiles=None, dates=None, days=None, products=None):
        ...
        # for each date, find all files (define self.tiles)
        self.numfiles = 0
        self.tiles = {}
        for date in sorted(dates):
            datedir = date.strftime('%Y%j')
            fnames = []
            for t in self.tile_names: fnames.extend( glob.glob(os.path.join(self.tilepath(t),datedir,'*.tar.gz')) )
            files = []
            for fname in fnames:
                ...
                for p in self.products:
                    pattern = os.path.join(product_path,'*'+p+'.tif')
                    product_fnames.extend( glob.glob(pattern) )
                products = [os.path.basename(f) for f in product_fnames]
                f = fname.split()
                e = {'tile': tile, 'color':self._colors['Landsat'+basename[2:3]],
                    'path':path, 'filename': fname, 'basename':basename[:-12], 
                    'products': sorted(products), 'productpath':product_path}
                files.append(e)
            self.numfiles = self.numfiles + len(fnames)
            self.tiles[date] = files
    ...
    def printcalendar(self,md=False,products=False):
        ...
        if self.site is not None:
            print 'Tile Coverage:'
            for t in sorted(self.tile_coverage): print ' P/R %s: %2.0f%%' % (t,self.tile_coverage[t]*100)
...
def process(inventory, products=['rad'], atmcorr=False, 
    verbose=0, overwrite=False, suffix='', overviews=False):

    print 'Requested %s products for %s files' % (len(products), inventory.numfiles)
    
    atmtag = ({True:'_A_', False: '_N_'})[atmcorr]
    if suffix != '' and suffix[:1] != '_': suffix = '_' + suffix

    for date in inventory.dates():
        for fin in inventory.tiles[date]:
            fout_base = os.path.join(fin['productpath'], fin['basename'] + atmtag)

            # Make output directory
            if not os.path.isdir(fin['productpath']):
                try:
                    os.makedirs(fin['productpath'])
                except OSError as exc:
                    raise Exception('Unable to make product directory %s' % fin['productpath'])
            ...
            for product in products:
                fout = fout_base + product.lower() + suffix
                if len(glob.glob(fout+'.*')) == 0 or overwrite: fouts[product.lower()] = fout
            ...
            if len(fouts) > 0:
                ...
                try:
                    ...
                    img = read(fin['filename'], verbose=verbose)
                    # TODO - shouldn't have to _readmeta again
                    meta = _readmeta(fin['filename'])
                except Exception,e:
                    print '%s %s' % (fin['basename'],e)
                ...
                if atmcorr:
                    for i in range(0,img.NumBands()):
                        ...
                        img[i] = band
                print '%s: read in %s' % (fin['basename'],datetime.datetime.now() - start)
...
if __name__ == "__main__":
    ...
    group.add_argument('-a','--atmcorr',action='store_true',help='Atmospheric Correction', default=False)
    ...
    if args.command == 'inventory':
        ...
        process(inv, products=args.products, atmcorr=args.atmcorr, 
            verbose=args.verbose, overwrite=args.overwrite, suffix=args.suffix) #, nooverviews=args.nooverviews)
